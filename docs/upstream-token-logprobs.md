...
- Raw server, Qwen3.5-2B-Q8_0, M4 Pro Metal: per-id values match an `n_probs=32768` readout from a separate request within 3.1e-5 for 4 ids (ranks 3, 30, 186, 28,732). The offset is equal for all ids, so it sits in the normalizer, not in the lookup. Median of 20: per-id 28.1 ms, `n_probs=256` 33.7 ms, `n_probs=32768` 99.1 ms. Bad id returns 400. The field also works on streamed `/completion` and on `/v1/chat/completions`.
